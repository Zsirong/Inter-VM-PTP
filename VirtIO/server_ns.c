...
#define Debug_LOG "/home/xiong/桌面/zsr/Inter-VM-PTP/VirtIO/debug_ns.log"
...
//        gettimeofday(&time_ns,NULL);
	...
	ret = write(sock_fd1,&time_ns,sizeof(struct timespec));
        if(ret == -1)
        {
...
//        write(fd_debug,buf_log,strlen(buf_log));
        ...
        //获取从钟第一次发送消息,此时为t4
	read(sock_fd1,&time_ns,sizeof(struct timeval));
